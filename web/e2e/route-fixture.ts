...
const ddosReport = JSON.parse(readFileSync(new URL('../tests/fixtures/ddos-report-contract.json', import.meta.url), 'utf8'));
...
export async function installApiFixture(page: Page) {
  ...
  await page.route('**/api/v1/**', async route => {
    ...
    if (path === '/analysis-jobs/ddos-job' && method === 'GET') return fulfill(route, { id: 'ddos-job', name: 'DDoS E2E', status: 'COMPLETED', packet_count: 1200, flow_count: 24, candidate_count: 0, sensor_ids: ['sensor-a'], analysis: { module: 'ddos_attack' }, ddos_attack: ddosReport });
    if (path === '/analysis-jobs/job-1/cancel') return fulfill(route, { status: 'CANCELLED' });
    if (path === '/pcap-analysis-jobs/initiate' && method === 'POST') return fulfill(route, { id: 'upload-job', name: 'Uploaded E2E capture', status: 'CREATED', processing: { phase: 'UPLOAD_PENDING' } }, 201);
    if (path === '/pcap-analysis-jobs/upload-job/capture' && method === 'PUT') return fulfill(route, { id: 'upload-job', name: 'Uploaded E2E capture', status: 'INGESTING', processing: { phase: 'UPLOAD_STORED' } }, 202);
    if (path === '/analysis-jobs/upload-job' && method === 'GET') return fulfill(route, { id: 'upload-job', name: 'Uploaded E2E capture', status: 'COMPLETED', source_type: 'PCAP_UPLOAD', processing: { phase: 'COMPLETED', phase_started_at: '2026-07-20T10:05:00Z' }, source: { filename: 'fixture.pcap', size_bytes: 4 }, packet_count: 1, flow_count: 1, candidate_count: 0 });
    ...
    if (path === '/pcap-exports/export-1' && method === 'GET') return fulfill(route, { id: 'export-1', status: 'COMPLETED', filename: 'candidate.pcap', exported_packet_count: 1, matched_packet_count: 1, truncated: false });
    if (path === '/pcap-exports/export-1/download' && method === 'GET') return route.fulfill({ status: 200, contentType: 'application/vnd.tcpdump.pcap', headers: { 'content-disposition': 'attachment; filename="candidate.pcap"' }, body: 'pcap-fixture' });
