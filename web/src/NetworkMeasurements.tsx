...
export default function NetworkMeasurements({ value }: { value: unknown }) {
  ...
  const stats = (v: unknown) => {
    ...
    return `${t('Samples', '표본')}: ${count(s.count)} · ${t('mean / min / max', '평균 / 최소 / 최대')}: ${[s.mean, s.min, s.max].map(n => number(measured ? n : null)).join(' / ')} · ${t('dispersion (population stddev)', '산포 (모집단 표준편차)')}: ${number(measured && (s.count as number) > 1 ? s.stddev : null)}`;
  };
  ...
  return <section aria-label={t('Supporting measurements', '보조 측정값')}>
    ...
    <p><strong>{t('Observed RTT (ms)', '관찰된 RTT (ms)')}</strong> — {stats(m.observed_rtt_ms)}</p>
    ...
    <p><strong>{t('Interarrival dispersion (ms)', '도착 간격 산포 (ms)')}</strong><br/>A → B: {stats(spacing.a_to_b)}<br/>B → A: {stats(spacing.b_to_a)}</p>
    ...
    <p><strong>{t('IPv4 TTL / IPv6 Hop Limit', 'IPv4 TTL / IPv6 홉 제한')}</strong>{['a_to_b', 'b_to_a'].map((direction, i) => {
      const s = object(ttl[direction]);
      const measured = typeof s.count === 'number' && s.count > 0;
      return <span key={direction}><br/>{i === 0 ? 'A → B' : 'B → A'}: {t('Samples', '표본')} {count(s.count)} · {t('min / max', '최소 / 최대')} {number(measured ? s.min : null)} / {number(measured ? s.max : null)} · {t('changes', '변화')} {count(s.changes)} · {t('missing', '누락')} {count(s.missing)}</span>;
    })}</p>
