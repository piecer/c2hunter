...
import DDoSAttackPanel, { type DDoSAttackReport } from './DDoSAttackPanel';
import DDoSAIInterpretation from './DDoSAIInterpretation';
...
type Page<T> = { items: T[]; page?: number; page_size?: number; total?: number; workflow_counts?: WorkflowCounts };
...
type CandidateVerdict = { id: string; verdict: 'CONFIRMED_C2' | 'FALSE_POSITIVE' | 'UNDER_REVIEW'; confidence: 'CONFIRMED' | 'LOW' | 'MEDIUM' | 'HIGH'; note: string; created_by: string; created_at: string };
type CandidateAction = { id: string; verdict_id: string; status: 'PENDING' | 'IN_PROGRESS' | 'COMPLETED'; note: string; created_by: string; created_at: string; completed_at?: string | null };
type ThreatIntelProvider = { status: 'PENDING' | 'OK' | 'ERROR' | 'AUTH_ERROR' | 'RATE_LIMITED' | 'NOT_CONFIGURED'; malicious?: number; suspicious?: number; harmless?: number; reputation?: number; abuse_confidence_score?: number; country_code?: string | null; total_reports?: number; attribute_count?: number; event_count?: number; matches?: Array<{ event_id?: string }>; error?: string; retry_after?: string };
type ThreatIntelligence = { ip_address?: string; candidate_ip?: string; fetched_at?: string; status?: 'PENDING' | 'COMPLETED' | 'PARTIAL' | 'FAILED'; origin?: 'AUTO' | 'MANUAL'; providers: Record<string, ThreatIntelProvider>; summary: { malicious?: number; suspicious?: number; harmless?: number; abuse_confidence_score?: number; misp_event_count?: number; country_code?: string | null; network_owner?: string | null } };
type ThreatIntelAssessment = { status: 'NOT_CHECKED' | 'PENDING' | 'COMPLETED' | 'PARTIAL' | 'FAILED'; signal: 'UNKNOWN' | 'INCOMPLETE' | 'NO_SIGNAL' | 'POSITIVE'; configured_providers: number; successful_providers: number; positive_providers: number; virustotal_malicious: number; virustotal_suspicious: number; abuse_confidence_score: number; misp_event_count: number; fetched_at?: string | null };
const threatIntelStatusLabel = (threatIntel: ThreatIntelligence) => {
  const prefix = threatIntel.origin === 'AUTO' ? '자동 조회' : '수동 조회';
  if (threatIntel.status === 'PENDING') return `${prefix} 중`;
  if (threatIntel.status === 'PARTIAL') return `${prefix} 부분 완료`;
  if (threatIntel.status === 'FAILED') return `${prefix} 실패`;
  return `${prefix} 완료`;
};
...
type IntegrationSettings = { version: number; virustotal_enabled: boolean; abuseipdb_enabled: boolean; misp_enabled: boolean; misp_url: string; misp_verify_tls: boolean; threat_intel_timeout_seconds: number; threat_intel_request_delay_seconds: number; abuseipdb_max_age_days: number; abuseipdb_positive_threshold: number; candidate_auto_enrichment_enabled: boolean; candidate_auto_enrichment_limit: number; candidate_auto_enrichment_workers: number; candidate_auto_enrichment_queue_capacity: number; management_event_id: string; management_auto_register: boolean; immediate_action_event_id: string; immediate_action_auto_register: boolean; immediate_action_min_positive_providers: number; updated_by?: string; updated_at?: string };
type Candidate = { id: string; job_id?: string; candidate_ip: string; score: number; severity: string; occurrence_count?: number; duplicate_count?: number; candidate_kind?: 'C2' | 'COMMUNICATION_STATUS'; distinct_internal_hosts?: number; hosts?: string[]; internal_hosts?: string[]; sensors?: string[]; sensor_ids?: string[]; protocols?: string[]; ports?: number[]; domains?: string[]; first_seen?: string; last_seen?: string; evidence?: Evidence[]; evidence_count?: number; adjustments?: ScoreAdjustment[]; traffic_series?: number[]; traffic_buckets?: TrafficBucket[]; related_attack_targets?: string[]; flow_count?: number; packet_count?: number; byte_count?: number; workflow_status?: 'NEEDS_REVIEW' | 'IN_REVIEW' | 'ACTION_REQUIRED' | 'ACTION_IN_PROGRESS' | 'ACTION_COMPLETED' | 'FALSE_POSITIVE'; action_status?: 'NOT_REQUIRED' | CandidateAction['status']; current_verdict?: CandidateVerdict; verdict_history?: CandidateVerdict[]; current_action?: CandidateAction; action_history?: CandidateAction[]; threat_intelligence?: ThreatIntelligence; ti_assessment?: ThreatIntelAssessment; misp_exports?: MispExport[] };
...
type DashboardData = {
  generated_at: string;
  fleet: { total: number; online: number; offline: number; degraded: number; dropped_packets: number };
  analyses: { total: number; active: number; completed_24h: number; failed_24h: number; partially_completed_24h: number; by_status: Record<string, number> };
  candidates: { total: number; communication_status: number; critical: number; high: number; medium: number; low: number; new_24h: number } & WorkflowCounts;
  candidate_trend: Array<{ hour: string; count: number }>;
  priority_candidates: Candidate[];
  recent_analyses: Job[];
  sensor_quality: Array<{ sensor_id: string; name: string; status: string; received_packets: number; dropped_packets: number; drop_rate_percent: number; last_heartbeat_at?: string; last_error?: string }>;
  attention: Array<{ kind: string; severity: string; title: string; detail: string; href: string }>;
};
...
const detectorLabels = {
  ...Object.fromEntries(detectorDefinitions.map(([name, label]) => [name, `${label} 탐지기`])),
  tcp_communication_attempt: 'TCP 통신 재시도 탐지기',
...
const evidenceLabels: Record<string, string> = {
  ...
  TCP_COMMUNICATION_ATTEMPT_PATTERN: 'TCP 통신 재시도 패턴',
  TCP_COMMUNICATION_ATTEMPT_ANALYSIS_INCOMPLETE: 'TCP 재시도 분석 불완전',
};
const metricLabels: Record<string, string> = {
  reason: '분석 제한 사유',
  outcome: '통신 결과',
  syn_transmission_count: 'SYN 전송 수',
  retransmission_count: '재전송 수',
  distinct_initial_sequence_count: '고유 초기 시퀀스 수',
  retry_intervals_ms: '재시도 간격(ms)',
  base_interval_ms: '기준 간격(ms)',
  response_timing_approximate: '응답 시각 근사',
  internal_host: '내부 호스트',
  internal_port: '내부 포트',
...
const adjustmentLabels: Record<string, string> = { SINGLE_HOST: '단일 호스트 감점', LOW_SAMPLE: '표본 부족 감점', PUBLIC_DNS_NTP: '공용 DNS·NTP 감점', CDN_CLOUD: 'CDN·클라우드 감점', HIGH_VOLUME: '대용량 통신 감점', HIGH_VOLUME_TCP_SESSION: '대용량 TCP 세션 감점' };
...
const metricLabel = (name: string) => metricLabels[name] ?? humanize(name);
...
const localizedMetricValue = (value: unknown) => {
  const labels: Record<string, string> = { EXACT: '정확 일치', STRUCTURAL: '구조 유사', FIRST: '첫 페이로드', LAST: '마지막 페이로드', NO_COMPLETION_OBSERVED: '세션 성립 미관찰', REFUSED_OR_RESET_OBSERVED: 'RST 응답 관찰', PEER_RESPONSE_OBSERVED: 'Peer 응답 관찰', LOCAL_ACK_OBSERVED: 'Local ACK 관찰', ESTABLISHED_OBSERVED: '세션 성립 관찰', SYN_TRANSMISSION_OBSERVATIONS: 'SYN 전송 관측', observation_budget_exceeded: '관측 예산 초과', episode_budget_exceeded: '에피소드 예산 초과', candidate_evidence_budget_exceeded: '후보 evidence 예산 초과', analysis_evidence_budget_exceeded: '분석 evidence 예산 초과', alert: '경고', monitor: '관찰', HIGH: '높음', LOW: '낮음' };
  return typeof value === 'string' && labels[value] ? labels[value] : formatValue(value);
};
const MetricValue = ({ name, value }: { name: string; value: unknown }) =>
...
const formatValue = (value: unknown): string => {
  ...
  if (typeof value === 'object') {
    const fields = Object.entries(value).map(([key, nested]) => `${humanize(key)}: ${formatValue(nested)}`);
    return fields.length ? fields.join(' · ') : 'None';
  }
  return String(value);
};

function DetectorWeightFields({ weights, setWeights, applyDefault = true, enabled = true }: { weights: DetectorWeights; setWeights: (value: DetectorWeights) => void; applyDefault?: boolean; enabled?: boolean }) {
  const queryClient = useQueryClient();
  const presets = useQuery<List<DetectorWeightPreset>, Error>({ queryKey: ['detector-weight-presets'], queryFn: () => api.get('/detector-weight-presets'), enabled });
  ...
  useEffect(() => {
    if (!enabled || !applyDefault || appliedDefault.current || !presets.data) return;
  ...
  }, [enabled, applyDefault, presets.data, setWeights]);
...
function AsyncState<T>({ query, children, empty }: { query: ReturnType<typeof useQuery<T, Error>>; children: (data: T) => ReactNode; empty?: (data: T) => boolean }) {
  if (query.isLoading) return <div className="state state-card loading" role="status" aria-live="polite"><span className="spinner"/><span><strong>데이터를 불러오는 중입니다</strong><small>최신 운영 정보를 확인하고 있습니다.</small></span></div>;
  if (query.isError) return <div className="state state-card error" role="alert"><span className="state-icon" aria-hidden="true">!</span><span><strong>데이터를 불러오지 못했습니다</strong><small>{query.error.message}</small></span><button type="button" onClick={() => query.refetch()}>다시 시도</button></div>;
  if (query.data && empty?.(query.data)) return <div className="state state-card empty"><span className="state-icon" aria-hidden="true">—</span><span><strong>표시할 데이터가 없습니다</strong><small>새 데이터가 수집되면 이 화면에 표시됩니다.</small></span></div>;
...
function Shell({ children }: { children: ReactNode }) {
  ...
  return <div className="shell"><aside onKeyDown={event => { if (event.key === 'Escape' && navigationOpen) { setNavigationOpen(false); navigationButton.current?.focus(); } }}><div className="brand">C2<span>Hunter</span></div><button ref={navigationButton} type="button" className="navigation-toggle secondary" aria-expanded={navigationOpen} aria-controls="shell-navigation" onClick={() => setNavigationOpen(!navigationOpen)}>메뉴 / Menu</button><div id="shell-navigation" className={`shell-navigation${navigationOpen ? ' is-open' : ''}`}><nav aria-label="Primary" onClick={event => { if ((event.target as HTMLElement).closest('a')) { setNavigationOpen(false); if (window.matchMedia('(max-width: 800px)').matches) navigationButton.current?.focus(); } }}><NavLink end to="/">Dashboard</NavLink><NavLink to="/sensors">Sensors</NavLink><NavLink to="/sensor-pcaps">Sensor PCAPs</NavLink><NavLink to="/external-sensors">External sensors</NavLink><NavLink className="nav-child" to="/external-sensors/enroll">Enroll sensor</NavLink><NavLink to="/analyses">Analysis history</NavLink><NavLink className="nav-child" to="/analyses/new">New analysis</NavLink><NavLink className="nav-child" to="/analyses/upload">Upload PCAP</NavLink><NavLink to="/candidates">Candidates</NavLink><NavLink to="/payload-signatures">Payload signatures</NavLink><NavLink to="/allowlist">Allowlist</NavLink><NavLink to="/settings/integrations">Integration settings</NavLink></nav><button className="quiet" onClick={() => { localStorage.removeItem('c2hunter-token'); navigate('/login'); }}>Sign out</button></div></aside><main className="content">{children}</main></div>;
}
...
function Dashboard() {
  ...
  return <><header className="header-actions"><div><p className="eyebrow">OPERATIONS OVERVIEW</p><h1>Dashboard</h1><p className="muted">센서 상태와 탐지 결과를 한눈에 확인하고 우선 조사 대상을 바로 처리합니다.</p></div><div className="actions"><Link className="button-link secondary-link" to="/analyses/upload">PCAP 업로드</Link><Link className="button-link" to="/analyses/new">새 분석</Link></div></header><AsyncState query={query}>{d => <div className="dashboard"><section className="metrics dashboard-metrics" aria-label="운영 핵심 지표"><article className="metric"><span>온라인 센서</span><strong>{d.fleet.online} / {d.fleet.total}</strong><small>오프라인 {d.fleet.offline} · 성능 저하 {d.fleet.degraded}</small></article><article className="metric"><span>통신 상태</span><strong>{d.candidates.communication_status}</strong><small>C2 위협 점수와 분리된 운영 관측</small></article><article className={`metric ${d.candidates.action_required + (d.candidates.action_in_progress ?? 0) ? 'threat' : ''}`}><span>조치 필요</span><strong>{d.candidates.action_required + (d.candidates.action_in_progress ?? 0)}</strong><small>대기 {d.candidates.action_required} · 진행 중 {d.candidates.action_in_progress ?? 0}</small></article><article className="metric"><span>분석 필요</span><strong>{d.candidates.needs_review}</strong><small>최근 24시간 신규 {d.candidates.new_24h}</small></article><article className="metric"><span>진행 중 분석</span><strong>{d.analyses.active}</strong><small>24시간 내 완료 {d.analyses.completed_24h}</small></article><article className={`metric ${d.analyses.failed_24h ? 'warning' : ''}`}><span>24시간 내 실패</span><strong>{d.analyses.failed_24h}</strong><small>전체 분석 {d.analyses.total}</small></article><article className={`metric ${d.fleet.dropped_packets ? 'warning' : ''}`}><span>센서 누락 패킷</span><strong>{d.fleet.dropped_packets.toLocaleString()}</strong><small>전체 센서 누적 보고값</small></article></section><section className="dashboard-overview-grid"><article className="panel"><div className="section-heading"><div><p className="eyebrow">THREAT TREND</p><h2>최근 24시간 후보 추세</h2></div><Link to="/candidates">전체 후보 보기</Link></div><MiniChart values={d.candidate_trend.map(bucket => bucket.count)} label="최근 24시간 시간별 신규 후보" /></article><article className="panel"><div className="section-heading"><div><p className="eyebrow">SEVERITY</p><h2>후보 심각도</h2></div><strong>{d.candidates.total}</strong></div><div className="severity-bars">{(['critical', 'high', 'medium', 'low'] as const).map(severity => { const value = d.candidates[severity]; const percent = d.candidates.total ? value / d.candidates.total * 100 : 0; return <div key={severity}><div><span className={`badge ${severity}`}>{severity.toUpperCase()}</span><strong>{value}</strong></div><span className={`severity-track ${severity}`}><span style={{ width: `${percent}%` }} /></span></div>; })}</div></article></section><section className="dashboard-action-grid"><article className="panel attention-panel"><div className="section-heading"><div><p className="eyebrow">ACTION QUEUE</p><h2>지금 확인할 항목</h2></div><strong>{d.attention.length}</strong></div>{d.attention.length ? <ul className="dashboard-list">{d.attention.map((item, index) => <li key={`${item.kind}-${item.href}-${index}`}><Link to={item.href}><span className={`badge ${item.severity.toLowerCase()}`}>{item.severity}</span><strong>{item.title}</strong><small>{item.detail}</small></Link></li>)}</ul> : <div className="state compact-state">즉시 확인할 운영 경고가 없습니다.</div>}</article><article className="panel"><div className="section-heading"><div><p className="eyebrow">REVIEW QUEUE</p><h2>분석 및 조치 필요 후보</h2></div><span className="queue-summary">분석 필요 {d.candidates.needs_review} · 분석 중 {d.candidates.in_review} · 조치 필요 {d.candidates.action_required}</span><Link to="/candidates">후보 목록</Link></div>{d.priority_candidates.length ? <ul className="dashboard-list candidates" aria-label="분석 및 조치 필요 후보">{d.priority_candidates.map(candidate => <li key={candidate.id}><Link to={`/candidates/${candidate.id}`}><span className={`badge ${candidate.severity.toLowerCase()}`}>{candidate.severity}</span><strong>{candidate.candidate_ip}</strong><small>점수 {candidate.score} · 근거 {candidate.evidence_count ?? candidate.evidence?.length ?? 0}개 · 마지막 관측 {fmt(candidate.last_seen)}</small></Link></li>)}</ul> : <div className="state compact-state">분석이 필요한 Candidate가 없습니다.</div>}</article></section><section className="dashboard-overview-grid"><article className="panel"><div className="section-heading"><div><p className="eyebrow">PIPELINE</p><h2>분석 파이프라인</h2></div><Link to="/analyses">전체 상태 보기</Link></div><div className="pipeline-status-grid">{([['WAITING_FOR_SENSOR', '센서 대기'], ['CAPTURING', '캡처'], ['UPLOADING', '업로드'], ['INGESTING', '적재'], ['ANALYZING', '분석']] as const).map(([status, label]) => <div key={status}><span>{label}</span><strong>{d.analyses.by_status[status] ?? 0}</strong></div>)}</div><p className="muted">최근 24시간 부분 완료 {d.analyses.partially_completed_24h}건 · 실패 {d.analyses.failed_24h}건</p></article><article className="panel"><div className="section-heading"><div><p className="eyebrow">COLLECTION QUALITY</p><h2>센서 수집 품질</h2></div><Link to="/sensors">전체 센서 보기</Link></div>{d.sensor_quality.length ? <ul className="dashboard-list">{d.sensor_quality.slice(0, 5).map(sensor => <li key={sensor.sensor_id}><Link to={`/sensors/${sensor.sensor_id}`}><span className={`badge ${sensor.status.toLowerCase()}`}>{sensor.status}</span><strong>{sensor.name}</strong><small>수신 {sensor.received_packets.toLocaleString()} · 누락 {sensor.dropped_packets.toLocaleString()} · 드롭률 {sensor.drop_rate_percent.toFixed(2)}%</small>{sensor.last_error && <small className="error-text">{sensor.last_error}</small>}</Link></li>)}</ul> : <div className="state compact-state">등록된 센서가 없습니다.</div>}</article></section><section className="panel"><div className="section-heading"><div><p className="eyebrow">RECENT ACTIVITY</p><h2>최근 분석</h2></div><Link to="/analyses">분석 기록 보기</Link></div>{d.recent_analyses.length ? <div className="table-wrap"><table aria-label="최근 분석"><thead><tr><th>분석</th><th>상태</th><th>후보</th><th>Packets / Flows</th><th>생성</th></tr></thead><tbody>{d.recent_analyses.map(job => <tr key={job.id}><td><Link to={`/analyses/${job.id}`}>{job.name}</Link></td><td><span className={`badge ${job.status.toLowerCase()}`}>{job.status}</span></td><td>{job.candidate_count ?? 0}</td><td>{(job.packet_count ?? 0).toLocaleString()} / {(job.flow_count ?? 0).toLocaleString()}</td><td>{fmt(job.created_at)}</td></tr>)}</tbody></table></div> : <div className="state compact-state">아직 생성된 분석이 없습니다.</div>}</section><p className="dashboard-updated">마지막 집계 {fmt(d.generated_at)} · 30초마다 자동 갱신</p></div>}</AsyncState></>;
}
...
function SensorPCAPTable({ segments, showAnalysis = true, label = 'Sensor PCAP archives' }: { segments: SensorPCAP[]; showAnalysis?: boolean; label?: string }) {
  const download = useMutation({ mutationFn: (segment: SensorPCAP) => api.download(`/sensor-pcaps/${segment.id}/download`, segment.filename) });
  return <><div className="table-wrap"><table aria-label={label}><thead><tr>{showAnalysis && <th>Analysis</th>}<th>Sensor</th><th>File</th><th>Size</th><th>Uploaded</th><th>Action</th></tr></thead><tbody>{segments.map(segment => <tr key={segment.id}>{showAnalysis && <td>{segment.analysis_job_id ? <Link to={`/analyses/${segment.analysis_job_id}`}>{segment.analysis_job_id}</Link> : 'Legacy capture'}</td>}<td>{segment.sensor_name ?? segment.sensor_id}</td><td><code>{segment.filename}</code></td><td>{formatBytes(segment.size_bytes)}</td><td>{fmt(segment.uploaded_at)}</td><td><button disabled={download.isPending} onClick={() => download.mutate(segment)}>{download.isPending && download.variables?.id === segment.id ? 'Downloading…' : `Download ${segment.filename}`}</button></td></tr>)}</tbody></table></div>{download.error && <p role="alert" className="error-text">{download.error.message}</p>}</>;
}

function SensorPCAPs() {
  const query = useQuery<List<SensorPCAP>, Error>({ queryKey: ['sensor-pcaps'], queryFn: () => api.get('/sensor-pcaps') });
  return <><header className="header-actions"><div><p className="eyebrow">CAPTURE ARCHIVE</p><h1>Sensor PCAPs</h1><p className="muted">Download finalized packet captures uploaded by enrolled sensors.</p></div></header>
    <AsyncState query={query}>{data => items(data).length ? <SensorPCAPTable segments={items(data)}/> : <div className="state state-card empty"><span className="state-icon" aria-hidden="true">—</span><span><strong>저장된 Sensor PCAP이 없습니다</strong><small>PCAP 저장을 켠 LIVE 분석이 종료되고 센서 업로드가 완료되면 표시됩니다.</small></span></div>}</AsyncState>
  </>;
}

function AnalysisPCAPs({ jobId }: { jobId: string }) {
  const query = useQuery<List<SensorPCAP>, Error>({ queryKey: ['sensor-pcaps', 'analysis', jobId], queryFn: () => api.get(`/sensor-pcaps?analysis_job_id=${encodeURIComponent(jobId)}&page_size=200`) });
  return <section className="panel compact analysis-pcaps" aria-labelledby="analysis-pcaps-heading"><div className="header-actions"><div><h2 id="analysis-pcaps-heading">분석 PCAP</h2><p className="muted">이 분석에서 저장하고 센서 업로드까지 완료한 packet capture입니다.</p></div><Link to="/sensor-pcaps">전체 PCAP 보기</Link></div><AsyncState query={query}>{data => items(data).length ? <SensorPCAPTable segments={items(data)} showAnalysis={false} label="Analysis PCAP archives"/> : <div className="state state-card empty"><span className="state-icon" aria-hidden="true">—</span><span><strong>다운로드 가능한 PCAP이 아직 없습니다</strong><small>PCAP 저장을 요청한 LIVE 분석은 종료·finalize·업로드가 완료된 뒤 파일이 표시됩니다.</small></span></div>}</AsyncState></section>;
}
...
function ExternalSensorConfiguration({ sensor, sensorId, reload }: { sensor: Sensor; sensorId: string; reload: () => void }) {
  ...
  const [sources, setSources] = useState<CaptureSource[]>(desired.capture_sources.map(source => ({ ...source, interface: source.interface ?? source.name ?? '', store_pcap: false })));
...
function AnalysisResult({ job }: { job: Job }) {
  if (job.analysis?.module === 'ddos_attack' && job.ddos_attack_summary) {
    const summary = job.ddos_attack_summary;
    const findingCount = typeof summary.finding_count === 'number' ? summary.finding_count : 0;
    return <><strong>{findingCount}</strong> DDoS findings<small>{String(summary.verdict ?? 'unknown')} · {String(summary.confidence ?? 'unknown')} confidence</small></>;
  }
  return <><strong>{job.candidate_count ?? 0}</strong> candidates<small>{job.packet_count ?? 0} packets · {job.flow_count ?? 0} flows</small></>;
}
...
function NewAnalysis() {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    mutation.mutate({
      ...
      analysis: form.get('analysis_module') === 'network_anomaly' ? { module: 'network_anomaly' } : isDDoS ? { module: 'ddos_attack', ...ddosParameters(form) } : { module: 'c2', profile: 'ddos_botnet', minimum_candidate_score: Number(form.get('score')), minimum_distinct_clients: Number(form.get('hosts')), ...(form.has('detector_weights_explicit') ? { detector_weights: detectorWeights } : {}), ml_anomaly_enabled: form.get('ml_anomaly_enabled') === 'on', ml_anomaly_allow_standalone: form.get('ml_anomaly_allow_standalone') === 'on' },
    });
  };
  return <><header><p className="eyebrow">INVESTIGATION</p><h1>New analysis</h1></header><form className="panel form" onSubmit={submit}><label>Analysis name<input name="name" required /></label><label>Analysis module<select name="analysis_module" value={analysisModule} onChange={event => setAnalysisModule(event.target.value)}><option value="c2">C2 detection</option><option value="ddos_attack">DDoS attack analysis</option><option value="network_anomaly">Network anomaly</option></select></label>{analysisModule === 'network_anomaly' && <NetworkObservationHelp/>}{isDDoS && <><DDoSAnalysisHelp/><DDoSThresholdFields/></>}<fieldset><legend>Sensors</legend><AsyncState query={sensors}>{data => <>{items(data).map(sensor => <label className="check" key={sensor.sensor_id}><input type="checkbox" name="sensor_ids" value={sensor.sensor_id} aria-label={sensor.name}/>{sensor.name}</label>)}</>}</AsyncState></fieldset><div className="grid"><label>Data source<select name="mode"><option value="LIVE">Live capture</option><option value="HISTORICAL">Historical</option></select></label><label>Duration (seconds)<input name="duration" type="number" min="1" defaultValue="300" /></label><label>Internal networks<input name="internal_networks" defaultValue="10.0.0.0/8" required /></label><label>Maximum packets<input name="max_packets" type="number" min="1" defaultValue="2000000" /></label><label>BPF filter<input name="bpf" defaultValue="ip" /></label></div><fieldset hidden={!isC2} disabled={!isC2} style={!isC2 ? { display: 'none' } : undefined}><legend>C2 candidate thresholds</legend><div className="grid"><label>Minimum score<input name="score" type="number" min="0" max="100" defaultValue="50" /></label><label>Minimum internal hosts<input name="hosts" type="number" min="2" defaultValue="3" /></label></div></fieldset><fieldset><legend>Directions</legend>{['INBOUND','OUTBOUND'].map(value => <label className="check" key={value}><input type="checkbox" name="directions" value={value} defaultChecked/>{value}</label>)}</fieldset><fieldset hidden={!isC2} disabled={!isC2} style={!isC2 ? { display: 'none' } : undefined}><DetectorWeightFields weights={detectorWeights} setWeights={setDetectorWeights} enabled={isC2}/><fieldset><legend>모집단 이상 탐지</legend><label className="check"><input type="checkbox" name="ml_anomaly_enabled"/>후보군 대비 이상 통신 탐지 사용</label><label className="check"><input type="checkbox" name="ml_anomaly_allow_standalone"/>이상 탐지만으로 후보 생성 허용</label><p className="muted">기본값은 사용 안 함입니다. 단독 후보 생성은 실험 기능이며, 허용하지 않으면 다른 탐지 근거가 있는 후보의 점수만 보강합니다.</p></fieldset></fieldset><label className="check"><input type="checkbox" name="store_pcap" defaultChecked/>Store PCAP</label>{mutation.error && <p role="alert">{mutation.error.message}</p>}<button disabled={mutation.isPending}>{mutation.isPending ? 'Starting…' : 'Start analysis'}</button></form></>;
}
...
function DDoSJobReanalysis({ job }: { job: Job }) {
  const navigate = useNavigate();
  const reanalyze = useMutation({
    mutationFn: (body: Record<string, unknown>) => api.post<Job>(`/analysis-jobs/${job.id}/reanalyze`, body),
    onSuccess: created => navigate(`/analyses/${created.id}`),
  });
  const submit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
  ...
  return <form className="panel compact form" onSubmit={submit}><h2>DDoS threshold 재분석</h2><p className="muted">원본 dataset {job.dataset_id}을 다시 수집하거나 파싱하지 않고 DDoS threshold snapshot만 변경합니다.</p><DDoSThresholdFields analysis={job.analysis}/>{reanalyze.error && <p role="alert" className="error-text">{reanalyze.error.message}</p>}<button disabled={reanalyze.isPending}>{reanalyze.isPending ? '재분석 생성 중…' : 'DDoS threshold로 재분석'}</button></form>;
}
...
function JobDetail() {
  ...
  const candidates = useQuery<List<Candidate>, Error>({
    ...
    enabled: q.data !== undefined && (q.data.analysis?.module ?? 'c2') === 'c2',
  ...
  return <AsyncState query={q}>{j => {
    ...
    return <>
      ...
      {(module === 'c2' || !terminal || ('error_code' in j && j.error_code === 'LIVE_CAPTURE_RESTART_INCOMPLETE') || strings(j.warnings).length > 0 || cancel.error || notice) && <section className="panel">
        ...
        {module === 'c2' && <><section className="metrics compact"><article><strong>{(j.packet_count ?? 0).toLocaleString()}</strong><span>Packets</span></article><article><strong>{(j.flow_count ?? 0).toLocaleString()}</strong><span>Flows</span></article>{j.analysis?.module === 'network_anomaly' ? <article><strong>{j.network_anomaly?.version === 'network-pattern-report-v1' ? (typeof j.network_anomaly.summary?.flow_count === 'number' ? j.network_anomaly.summary.flow_count.toLocaleString() : '—') : j.network_anomaly?.flows?.length.toLocaleString() ?? '—'}</strong><span>Observed bidirectional flows</span></article> : String(module) === 'ddos_attack' ? <article><strong>{Number(j.ddos_attack?.summary.finding_count ?? 0).toLocaleString()}</strong><span>DDoS findings</span></article> : <article><strong>{(j.candidate_count ?? 0).toLocaleString()}</strong><span>Candidates</span></article>}<article><strong>{strings(j.sensor_ids).length}</strong><span>Sensors</span></article></section></>}
        {'error_code' in j && j.error_code === 'LIVE_CAPTURE_RESTART_INCOMPLETE' && <div role="alert" aria-label="Incomplete LIVE capture" className="warning"><p lang="ko">컨트롤러 재시작으로 데이터셋 저장 전에 LIVE 캡처가 중단되었습니다. 수락된 메모리 내 flow batch가 유실되었을 수 있습니다. 새 캡처를 시작하세요. 표시된 0건은 트래픽이 없었다는 뜻이 아닙니다.</p><p lang="en">Controller restart interrupted LIVE capture before its dataset was saved; accepted in-memory flow batches may have been lost. Start a new capture. Displayed zero counts do not mean no traffic was observed.</p></div>}
      ...
      {module === 'network_anomaly' ? <NetworkAnomalyPanel key={j.id} report={j.network_anomaly}><NetworkAIInterpretation key={j.id} jobId={j.id} completed={j.status === 'COMPLETED' && Boolean(j.network_anomaly)}/></NetworkAnomalyPanel> : module === 'ddos_attack' && j.ddos_attack ? <DDoSAttackPanel key={`${j.id}:${j.ddos_attack.primary_finding_id ?? 'none'}:${String(j.ddos_attack.summary?.finding_count ?? 'unknown')}`} report={j.ddos_attack} detailOwner={detailsOwner} setDetailOwner={setDetailsOwner}><DDoSAIInterpretation jobId={j.id} completed={j.status === 'COMPLETED'}/></DDoSAttackPanel> : module === 'c2' ? <AIAnalysisPanel job={j}/> : <p role="alert">Unsupported analysis module.</p>}
      {module !== 'c2' && <button type="button" className="secondary" aria-expanded={detailsOwner === j.id} onClick={() => setDetailsOwner(detailsOwner === j.id ? undefined : j.id)}>작업 상세 / Job details</button>}
      {(module === 'c2' || detailsOwner === j.id) && <>
      {module !== 'c2' && <><section className="metrics compact"><article><strong>{(j.packet_count ?? 0).toLocaleString()}</strong><span>Packets</span></article><article><strong>{(j.flow_count ?? 0).toLocaleString()}</strong><span>Flows</span></article>{j.analysis?.module === 'network_anomaly' ? <article><strong>{j.network_anomaly?.version === 'network-pattern-report-v1' ? (typeof j.network_anomaly.summary?.flow_count === 'number' ? j.network_anomaly.summary.flow_count.toLocaleString() : '—') : j.network_anomaly?.flows?.length.toLocaleString() ?? '—'}</strong><span>Observed bidirectional flows</span></article> : String(module) === 'ddos_attack' ? <article><strong>{Number(j.ddos_attack?.summary.finding_count ?? 0).toLocaleString()}</strong><span>DDoS findings</span></article> : <article><strong>{(j.candidate_count ?? 0).toLocaleString()}</strong><span>Candidates</span></article>}<article><strong>{strings(j.sensor_ids).length}</strong><span>Sensors</span></article></section></>}
      ...
      <AnalysisConfiguration analysis={j.analysis} capture={j.capture} detectorDefinitions={detectorDefinitions} internalNetworks={strings(j.internal_networks)} sensorIds={strings(j.sensor_ids)}/>
      <AnalysisPCAPs jobId={j.id}/>
      </>}
      {j.status === 'COMPLETED' && j.dataset_id && (module === 'c2' ? <JobReanalysis key={j.id} job={j}/> : module === 'ddos_attack' ? <DDoSJobReanalysis key={j.id} job={j}/> : null)}
      {module === 'c2' && <section className="panel compact"><h2>탐지 후보</h2><AsyncState query={candidates} empty={data => items(data).length === 0}>{data => <div className="table-wrap"><table aria-label="Analysis candidates"><thead><tr><th>후보</th><th>점수</th><th>External TI</th><th>판정</th><th>호스트 / 센서</th><th>네트워크 정보</th><th>탐지 근거</th><th>관측 시각</th></tr></thead><tbody>{items(data).map(candidate => { const hosts = candidateHosts(candidate); const sensors = candidateSensors(candidate); const protocols = strings(candidate.protocols); const ports = numbers(candidate.ports); const evidence = candidateEvidence(candidate); return <tr key={candidate.id}><td><Link to={`/candidates/${candidate.id}`}>{candidate.candidate_ip}</Link><CandidateTypeBadge candidate={candidate}/><small className={candidate.severity.toLowerCase()}>{candidate.severity}</small></td><td><strong>{candidate.score}</strong></td><td><CandidateThreatIntel assessment={candidate.ti_assessment} /></td><td><CandidateWorkflowBadge candidate={candidate} /></td><td>{hosts.length || candidate.distinct_internal_hosts || 0}개 호스트<small>{sensors.length}개 센서</small></td><td>{protocols.length ? protocols.join(', ') : '알 수 없는 프로토콜'}<small>{ports.length ? `포트 ${ports.join(', ')}` : '서비스 포트 없음'}</small></td><td>{candidate.evidence_count ?? evidence.length}건<small>{evidence.map(item => evidenceLabel(item.type)).join(', ') || '상세 근거 없음'}</small></td><td>{fmt(candidate.first_seen)}<small>~ {fmt(candidate.last_seen)}</small></td></tr>; })}</tbody></table></div>}</AsyncState></section>}
      {module === 'c2' && <JobFlowReviewPanel key={`flow-review-${j.id}`} jobId={j.id} />}
      {(module === 'c2' || detailsOwner === j.id) && <><section className="panel compact"><h2>State transitions</h2>{j.transitions?.length ? <ol className="timeline">{j.transitions.map((transition, index) => <li key={`${transition.to_status}-${transition.occurred_at}-${index}`}><strong>{transition.to_status}</strong><span>{fmt(transition.occurred_at)}</span><p>{transition.reason ?? 'No transition reason recorded'}{transition.from_status ? ` · from ${transition.from_status}` : ''}</p></li>)}</ol> : <p className="muted">No state transition history was recorded.</p>}</section></>}
    </>;
  }}</AsyncState>;
}

const workflowPresentation = {
  NEEDS_REVIEW: { label: '미분석', description: '분석 필요', className: 'needs-review' },
  IN_REVIEW: { label: '분석 중', description: '분석가 검토 중', className: 'in-review' },
  ACTION_REQUIRED: { label: '조치 필요', description: 'C2 확인 후 대응 필요', className: 'action-required' },
  ACTION_IN_PROGRESS: { label: '조치 중', description: '후속 대응 진행 중', className: 'action-in-progress' },
  ACTION_COMPLETED: { label: '조치 완료', description: '후속 대응 완료', className: 'action-completed' },
  FALSE_POSITIVE: { label: '오탐 처리 완료', description: '오탐 판정 완료', className: 'false-positive' },
} as const;

function CandidateWorkflowBadge({ candidate }: { candidate: Candidate }) {
  const fallback = candidate.current_verdict?.verdict === 'CONFIRMED_C2' ? 'ACTION_REQUIRED' : candidate.current_verdict?.verdict === 'FALSE_POSITIVE' ? 'FALSE_POSITIVE' : candidate.current_verdict?.verdict === 'UNDER_REVIEW' ? 'IN_REVIEW' : 'NEEDS_REVIEW';
  const presentation = workflowPresentation[candidate.workflow_status ?? fallback];
  return <span className={`workflow-badge ${presentation.className}`} title={presentation.description}>{presentation.label}</span>;
}

function CandidateTypeBadge({ candidate }: { candidate: Candidate }) {
  return candidate.candidate_kind === 'COMMUNICATION_STATUS' ? <small>통신 상태 후보</small> : null;
}

function CandidateThreatIntel({ assessment }: { assessment?: ThreatIntelAssessment }) {
  if (!assessment || assessment.signal === 'UNKNOWN') return <div className="candidate-ti unknown"><strong>미조회</strong><small>외부 TI 정보 없음</small></div>;
  const signals = <div className="ti-signal-row">
    {assessment.misp_event_count > 0 && <span>MISP {assessment.misp_event_count}</span>}
    {(assessment.virustotal_malicious > 0 || assessment.virustotal_suspicious > 0) && <span>VT {assessment.virustotal_malicious}/{assessment.virustotal_suspicious}</span>}
    {assessment.abuse_confidence_score > 0 && <span>Abuse {assessment.abuse_confidence_score}%</span>}
  </div>;
  if (assessment.signal === 'INCOMPLETE') {
    const status = assessment.status === 'PENDING' ? '조회 중' : assessment.status === 'FAILED' ? '조회 실패' : '부분 완료';
    return <div className="candidate-ti incomplete"><strong>정보 부족</strong><small>{status}</small>{signals}<small>{assessment.successful_providers}/{assessment.configured_providers} 조회 · {assessment.positive_providers}개 양성</small></div>;
  }
  if (assessment.signal === 'NO_SIGNAL') return <div className="candidate-ti no-signal"><strong>외부 신호 없음</strong><small>{assessment.successful_providers}/{assessment.configured_providers} 조회 완료 · 안전 판정 아님</small></div>;
  return <div className="candidate-ti positive">
    {signals}
    <small>{assessment.successful_providers}/{assessment.configured_providers} 조회 · {assessment.positive_providers}개 양성</small>
  </div>;
}

function CandidateWorkflowSummary({ counts, onSelect }: { counts: WorkflowCounts; onSelect?: (workflowStatus: string) => void }) {
  const entries = [
    ['NEEDS_REVIEW', '미분석', counts.needs_review, 'needs-review'],
    ['IN_REVIEW', '분석 중', counts.in_review, 'in-review'],
    ['ACTION_REQUIRED', '조치 필요', counts.action_required, 'action-required'],
    ['ACTION_IN_PROGRESS', '조치 중', counts.action_in_progress ?? 0, 'action-in-progress'],
    ['ACTION_COMPLETED', '조치 완료', counts.action_completed ?? 0, 'action-completed'],
    ['FALSE_POSITIVE', '오탐 처리 완료', counts.false_positive ?? 0, 'false-positive'],
  ] as const;
  return <section className="workflow-summary" aria-label="Candidate 처리 현황">{entries.map(([workflowStatus, label, count, className]) => {
    const content = <><span>{label}</span><strong>{count}</strong></>;
    return onSelect
      ? <button type="button" className={className} key={workflowStatus} onClick={() => onSelect(workflowStatus)}>{content}</button>
      : <article className={className} key={workflowStatus}>{content}</article>;
  })}</section>;
}

type CandidateFilters = { severity: string; workflowStatus: string; minimumScore: string; tiFilter: string; includeSuppressed: boolean; sort: string };
const defaultCandidateFilters: CandidateFilters = { severity: '', workflowStatus: '', minimumScore: '0', tiFilter: '', includeSuppressed: false, sort: '-last_seen' };
...
function Candidates() {
  ...
  const [bulkCommand, setBulkCommand] = useState<CandidateVerdict['verdict']>('UNDER_REVIEW');
  ...
  const q = useQuery<Page<Candidate>, Error>({
    ...
    queryFn: () => {
      ...
      if (filters.workflowStatus) parameters.set('workflow_status', filters.workflowStatus);
      if (filters.tiFilter) parameters.set('ti_filter', filters.tiFilter);
      if (filters.includeSuppressed) parameters.set('include_suppressed', 'true');
      return api.get(`/candidates?${parameters}`);
    },
  });
  const bulk = useMutation({
    mutationFn: () => api.post<{ succeeded: number; failed: number }>('/candidate-bulk-operations', {
      candidate_ids: [...selected], command: bulkCommand, confidence: 'HIGH', note: bulkNote,
    }),
    onSuccess: async result => {
      setBulkNotice(`${result.succeeded}건 처리 완료${result.failed ? ` · ${result.failed}건 실패` : ''}`);
      setSelected(new Set());
      await q.refetch();
    },
  });
  const clearSelection = () => setSelected(new Set());
  const applyFilters = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); clearSelection(); setPage(1); setFilters({ ...draft, minimumScore: draft.minimumScore || '0' }); };
  const resetFilters = () => { clearSelection(); setDraft(defaultCandidateFilters); setFilters(defaultCandidateFilters); setPage(1); };
  const selectWorkflow = (workflowStatus: string) => {
    const selected = { ...filters, workflowStatus };
    setDraft(selected);
    setFilters(selected);
    clearSelection();
    setPage(1);
  };
  return <><header><p className="eyebrow">DETECTION RESULTS</p><h1>Candidates</h1><p className="muted">내부 탐지 점수와 외부 TI 근거를 분리해 비교하고 우선 조사 대상을 선택합니다.</p></header>{q.data?.workflow_counts && <CandidateWorkflowSummary counts={q.data.workflow_counts} onSelect={selectWorkflow} />}<form className="candidate-filters" onSubmit={applyFilters}><label>Severity<select value={draft.severity} onChange={event => setDraft({ ...draft, severity: event.target.value })}><option value="">Any</option><option>CRITICAL</option><option>HIGH</option><option>MEDIUM</option><option>LOW</option></select></label><label>처리 상태<select value={draft.workflowStatus} onChange={event => setDraft({ ...draft, workflowStatus: event.target.value })}><option value="">전체 상태</option><option value="NEEDS_REVIEW">미분석</option><option value="IN_REVIEW">분석 중</option><option value="ACTION_REQUIRED">조치 필요</option><option value="ACTION_IN_PROGRESS">조치 중</option><option value="ACTION_COMPLETED">조치 완료</option><option value="FALSE_POSITIVE">오탐 처리 완료</option></select></label><label>Minimum score<input type="number" min="0" max="100" value={draft.minimumScore} onChange={event => setDraft({ ...draft, minimumScore: event.target.value })} /></label><label>External TI<select value={draft.tiFilter} onChange={event => setDraft({ ...draft, tiFilter: event.target.value })}><option value="">전체</option><option value="POSITIVE">양성 신호만</option><option value="MISP_MATCH">MISP 일치만</option><option value="INCOMPLETE">정보 부족만</option></select></label><label>Sort candidates<select value={draft.sort} onChange={event => setDraft({ ...draft, sort: event.target.value })}><option value="-ti_priority">외부 TI 근거 강한 순</option><option value="-score">Score: high to low</option><option value="score">Score: low to high</option><option value="candidate_ip">IP address</option><option value="-last_seen">Most recently seen</option><option value="first_seen">First seen</option></select></label><label className="check"><input type="checkbox" checked={draft.includeSuppressed} onChange={event => setDraft({ ...draft, includeSuppressed: event.target.checked })} />Include suppressed candidates</label><button>Apply filters</button><button type="button" className="secondary" onClick={resetFilters}>Reset</button></form>{selected.size > 0 && <form className="panel compact bulk-toolbar" aria-label="Candidate 일괄 처리" onSubmit={event => { event.preventDefault(); bulk.mutate(); }}><strong>{selected.size}개 선택</strong><label>빠른 판정<select value={bulkCommand} onChange={event => setBulkCommand(event.target.value as CandidateVerdict['verdict'])}><option value="UNDER_REVIEW">검토 시작</option><option value="CONFIRMED_C2">확정 C2</option><option value="FALSE_POSITIVE">오탐</option></select></label><label>공통 메모<input required maxLength={5000} value={bulkNote} onChange={event => setBulkNote(event.target.value)} /></label><button disabled={!bulkNote.trim() || bulk.isPending}>{bulk.isPending ? '처리 중…' : '선택 처리'}</button><button type="button" className="secondary" onClick={clearSelection}>선택 해제</button>{bulk.error && <p role="alert" className="error-text">{bulk.error.message}</p>}</form>}{bulkNotice && <p role="status">{bulkNotice}</p>}<AsyncState query={q} empty={data => items(data).length === 0}>{data => <><div className="table-wrap"><table aria-label="C2 candidates"><thead><tr><th><input type="checkbox" aria-label="현재 페이지 전체 선택" checked={items(data).length > 0 && items(data).every(candidate => selected.has(candidate.id))} onChange={event => setSelected(event.target.checked ? new Set(items(data).map(candidate => candidate.id)) : new Set())} /></th><th>Candidate</th><th>Score</th><th>External TI</th><th>처리 상태</th><th>Hosts</th><th>Sensors</th><th>Protocol / port</th><th>Observed</th><th>Primary evidence</th></tr></thead><tbody>{items(data).map(candidate => { const hosts = candidateHosts(candidate); const sensors = candidateSensors(candidate); const protocols = strings(candidate.protocols); const ports = numbers(candidate.ports); const evidence = candidateEvidence(candidate); return <tr key={candidate.id}><td><input type="checkbox" aria-label={`${candidate.candidate_ip} 선택`} checked={selected.has(candidate.id)} onChange={event => setSelected(current => { const next = new Set(current); if (event.target.checked) next.add(candidate.id); else next.delete(candidate.id); return next; })} /></td><td><Link to={`/candidates/${candidate.id}`}>{candidate.candidate_ip}</Link><CandidateTypeBadge candidate={candidate}/><small className={candidate.severity.toLowerCase()}>{candidate.severity}</small>{(candidate.occurrence_count ?? 1) > 1 && <small>{candidate.occurrence_count}회 탐지 · 중복 {candidate.duplicate_count ?? (candidate.occurrence_count ?? 1) - 1}건 통합</small>}</td><td><strong>{candidate.score}</strong></td><td><CandidateThreatIntel assessment={candidate.ti_assessment} /></td><td><CandidateWorkflowBadge candidate={candidate} /></td><td>{hosts.length || candidate.distinct_internal_hosts || 0}</td><td>{sensors.length}</td><td>{protocols.length ? protocols.join(', ') : 'Unknown'}<small>{ports.length ? `Ports ${ports.join(', ')}` : 'No ports reported'}</small></td><td>{fmt(candidate.first_seen)}<small>to {fmt(candidate.last_seen)}</small></td><td title={evidence[0]?.type}>{evidenceLabel(evidence[0]?.type)}<small>{evidence.length}건의 탐지 신호</small></td></tr>; })}</tbody></table></div><CandidatePagination data={data} page={page} onPage={next => { clearSelection(); setPage(next); }} /></>}</AsyncState></>;
}
...
function CandidateEnrichmentPanel({ candidate, onUpdated }: { candidate: Candidate; onUpdated: () => Promise<unknown> }) {
  const [verdict, setVerdict] = useState<CandidateVerdict['verdict']>('UNDER_REVIEW');
  const [confidence, setConfidence] = useState<CandidateVerdict['confidence']>('MEDIUM');
  const [note, setNote] = useState('');
  const [actionNote, setActionNote] = useState('');
  const [eventId, setEventId] = useState('');
  const [notice, setNotice] = useState('');
  const verdictMutation = useMutation({
    mutationFn: () => api.post(`/candidates/${candidate.id}/verdicts`, { verdict, confidence, note: note.trim() }),
    onSuccess: async () => {
      setNotice('Candidate verdict recorded');
      setNote('');
      await onUpdated();
    },
  });
  const actionMutation = useMutation({
    mutationFn: (status: 'IN_PROGRESS' | 'COMPLETED') => api.post(`/candidates/${candidate.id}/actions`, { status, note: actionNote.trim() }),
    onSuccess: async () => {
      setNotice('후속 대응 조치 상태가 저장되었습니다');
      setActionNote('');
      await onUpdated();
    },
  });
  const threatIntelMutation = useMutation({
    mutationFn: () => api.post(`/candidates/${candidate.id}/threat-intelligence/lookups`, {}),
    onSuccess: async () => {
      setNotice('Threat intelligence lookup completed');
      await onUpdated();
    },
  });
  const mispMutation = useMutation({
    mutationFn: () => api.post(
      `/candidates/${candidate.id}/misp-exports`,
      eventId.trim() ? { event_id: eventId.trim() } : {},
    ),
    onSuccess: async () => {
      setNotice('MISP ip-src attribute exported');
      await onUpdated();
    },
  });
  const threatIntel = candidate.threat_intelligence;
  const verdictHistory = candidate.verdict_history ?? [];
  ...
  return <section className="panel compact candidate-enrichment" aria-labelledby="candidate-validation-title">
    ...
    <div className="grid compact">
      <form className="subpanel" onSubmit={event => { event.preventDefault(); verdictMutation.mutate(); }}>
        ...
        <label>Candidate verdict<select value={verdict} onChange={event => setVerdict(event.target.value as CandidateVerdict['verdict'])}><option value="UNDER_REVIEW">Under review</option><option value="CONFIRMED_C2">Confirmed C2</option><option value="FALSE_POSITIVE">False positive</option></select></label>
        <label>Verdict confidence<select value={confidence} onChange={event => setConfidence(event.target.value as CandidateVerdict['confidence'])}><option value="LOW">Low</option><option value="MEDIUM">Medium</option><option value="HIGH">High</option><option value="CONFIRMED">Confirmed</option></select></label>
        <label>Verdict note<textarea required minLength={1} maxLength={2000} value={note} onChange={event => setNote(event.target.value)} placeholder="Record the evidence supporting this verdict" /></label>
        <button disabled={!note.trim() || verdictMutation.isPending}>{verdictMutation.isPending ? '저장 중…' : '판정 저장'}</button>
      </form>
      <div className="subpanel">
        <div className="section-heading"><div><h3>외부 위협 인텔리전스</h3>{threatIntel && <span className={`workflow-badge ti-${(threatIntel.status ?? 'COMPLETED').toLowerCase()}`}>{threatIntelStatusLabel(threatIntel)}</span>}</div><button className="secondary" disabled={threatIntelMutation.isPending} onClick={() => threatIntelMutation.mutate()}>{threatIntelMutation.isPending ? '조회 중…' : '외부 TI 다시 조회'}</button></div>
        <p className="analyst-guardrail">외부 평판은 판단 근거이며 자동 확정하지 않습니다. 네트워크 행위 근거와 함께 분석가가 Confirm 여부를 결정하세요.</p>
        {!threatIntel && <p className="muted">구성된 VirusTotal, AbuseIPDB, MISP를 후보 생성 직후 자동 조회합니다.</p>}
        {threatIntel && <><p className="muted">{threatIntel.status === 'PENDING' ? `조회 요청 ${fmt(threatIntel.fetched_at)}` : `최근 조회 ${fmt(threatIntel.fetched_at)}`}</p><div className="ti-grid">{Object.entries(threatIntel.providers).map(([provider, result]) => <article key={provider} className="ti-card"><strong>{provider === 'virustotal' ? 'VirusTotal' : provider === 'abuseipdb' ? 'AbuseIPDB' : provider === 'misp' ? 'MISP' : provider}</strong>{result.status === 'PENDING' ? <p className="muted">조회 대기 중</p> : result.status === 'NOT_CONFIGURED' ? <p className="muted">Provider 설정 없음</p> : result.status !== 'OK' ? <p className="error-text">{result.error ?? 'Provider lookup failed'}{result.retry_after && ` · Retry-After ${result.retry_after}s`}</p> : provider === 'virustotal' ? <dl><dt>악성</dt><dd>악성 {result.malicious ?? 0}</dd><dt>의심</dt><dd>{result.suspicious ?? 0}</dd><dt>평판</dt><dd>{result.reputation ?? 0}</dd></dl> : provider === 'abuseipdb' ? <dl><dt>Abuse 신뢰도</dt><dd>Abuse 신뢰도 {result.abuse_confidence_score ?? 0}%</dd><dt>신고 수</dt><dd>{result.total_reports ?? 0}</dd><dt>국가</dt><dd>{result.country_code ?? 'Unknown'}</dd></dl> : <dl><dt>일치 Attribute</dt><dd>최대 100건 중 {result.attribute_count ?? 0}개</dd><dt>이벤트</dt><dd>MISP 이벤트 {result.event_count ?? 0}개</dd><dt>Event ID</dt><dd>{result.matches?.map(match => match.event_id).filter(Boolean).join(', ') || '없음'}</dd></dl>}</article>)}</div></>}
      </div>
    </div>
    {currentVerdict === 'CONFIRMED_C2' && <section className="subpanel candidate-action-panel" aria-labelledby="candidate-action-title">
      ...
      <h4>조치 이력</h4>
      {actionHistory.length === 0 ? <p className="muted">기록된 후속 조치가 없습니다.</p> : <ul className="history-list">{[...actionHistory].reverse().map(item => <li key={item.id}><strong>{item.status === 'PENDING' ? '조치 필요' : item.status === 'IN_PROGRESS' ? '조치 중' : '조치 완료'}</strong><span>{fmt(item.created_at)} · {item.created_by}</span><p>{item.note}</p></li>)}</ul>}
    </section>}
    <div className="grid compact">
      <form className="subpanel" onSubmit={event => { event.preventDefault(); mispMutation.mutate(); }}>
...
function CandidateDetail() {
  ...
  return <AsyncState query={q}>{candidate => {
    ...
    return <>
      <header className="header-actions"><div><p className="eyebrow">CANDIDATE DETAIL</p><h1>{candidate.candidate_ip}</h1><span className={`badge ${candidate.severity.toLowerCase()}`}>{candidate.score} · {candidate.severity}</span>{candidate.candidate_kind === 'COMMUNICATION_STATUS' && <span className="badge">통신 상태 후보</span>}<p className="record-id">Candidate {candidate.id}</p></div>{candidate.job_id && <Link to={`/analyses/${candidate.job_id}`}>View source analysis</Link>}</header>
...
function IntegrationSettingsPage() {
  const q = useQuery<IntegrationSettings, Error>({ queryKey: ['integration-settings'], queryFn: () => api.get('/integration-settings') });
  const [draft, setDraft] = useState<IntegrationSettings | null>(null);
  useEffect(() => { if (q.data && !draft) setDraft(q.data); }, [q.data, draft]);
  const save = useMutation({
    mutationFn: (value: IntegrationSettings) => {
      const version = value.version;
      const settings: Partial<IntegrationSettings> = { ...value };
      delete settings.version;
      delete settings.updated_by;
      delete settings.updated_at;
      return api.put<IntegrationSettings>('/integration-settings', { ...settings, expected_version: version });
    },
    onSuccess: value => { setDraft(value); void q.refetch(); },
  });
  return <><header><p className="eyebrow">ADMINISTRATION</p><h1>Threat intelligence & MISP</h1><p className="muted">API key는 배포 환경에서만 관리합니다. 이 화면은 공개 운영 정책과 두 MISP Event만 저장합니다.</p></header><AsyncState query={q}>{() => draft ? <form className="panel form" aria-label="외부 TI 및 MISP 설정" onSubmit={event => { event.preventDefault(); save.mutate(draft); }}><fieldset><legend>공급자 정책</legend><label className="check"><input type="checkbox" checked={draft.virustotal_enabled} onChange={event => setDraft({ ...draft, virustotal_enabled: event.target.checked })} />VirusTotal 사용</label><label className="check"><input type="checkbox" checked={draft.abuseipdb_enabled} onChange={event => setDraft({ ...draft, abuseipdb_enabled: event.target.checked })} />AbuseIPDB 사용</label><label>AbuseIPDB 양성 기준<input type="number" min="0" max="100" value={draft.abuseipdb_positive_threshold} onChange={event => setDraft({ ...draft, abuseipdb_positive_threshold: Number(event.target.value) })} /></label><label className="check"><input type="checkbox" checked={draft.misp_enabled} onChange={event => setDraft({ ...draft, misp_enabled: event.target.checked })} />MISP 사용</label><label>MISP URL<input value={draft.misp_url} onChange={event => setDraft({ ...draft, misp_url: event.target.value })} /></label><label className="check"><input type="checkbox" checked={draft.misp_verify_tls} onChange={event => setDraft({ ...draft, misp_verify_tls: event.target.checked })} />MISP TLS 인증서 검증</label><label>외부 TI 요청 간격(초)<input type="number" min="0" max="60" step="0.1" value={draft.threat_intel_request_delay_seconds} onChange={event => setDraft({ ...draft, threat_intel_request_delay_seconds: Number(event.target.value) })} /><small>TI와 MISP 요청은 한 번에 하나씩 실행되며, 완료 후 이 간격만큼 대기합니다.</small></label></fieldset><fieldset><legend>Candidate 자동화</legend><label>관리 Event ID<input value={draft.management_event_id} onChange={event => setDraft({ ...draft, management_event_id: event.target.value })} /></label><label className="check"><input type="checkbox" checked={draft.management_auto_register} onChange={event => setDraft({ ...draft, management_auto_register: event.target.checked })} />Candidate 생성 시 관리 Event 자동 등록</label><label>즉시조치 Event ID<input value={draft.immediate_action_event_id} onChange={event => setDraft({ ...draft, immediate_action_event_id: event.target.value })} /></label><label>즉시조치 최소 양성 공급자<input type="number" min="2" max="3" value={draft.immediate_action_min_positive_providers} onChange={event => setDraft({ ...draft, immediate_action_min_positive_providers: Number(event.target.value) })} /></label><label className="check"><input type="checkbox" checked={draft.immediate_action_auto_register} onChange={event => setDraft({ ...draft, immediate_action_auto_register: event.target.checked })} />자동 CONFIRMED_C2 및 즉시조치 Event 등록</label></fieldset><button disabled={save.isPending}>{save.isPending ? '저장 중…' : '설정 저장'}</button>{save.isSuccess && <p role="status">설정이 저장되었습니다.</p>}{save.error && <p role="alert" className="error-text">{save.error.message}</p>}</form> : <div className="state">설정을 준비하는 중입니다.</div>}</AsyncState></>;
}
