...
function JobDetail() {
  ...
  return <AsyncState query={q}>{j => {
    ...
    return <>
      ...
      {module === 'network_anomaly' ? <NetworkAnomalyPanel key={j.id} report={j.network_anomaly}><NetworkAIInterpretation key={j.id} jobId={j.id} completed={j.status === 'COMPLETED' && Boolean(j.network_anomaly)}/></NetworkAnomalyPanel> : module === 'ddos_attack' && j.ddos_attack ? <DDoSAttackPanel key={`${j.id}:${j.ddos_attack.primary_finding_id ?? 'none'}:${String(j.ddos_attack.summary?.finding_count ?? 'unknown')}`} report={j.ddos_attack} detailOwner={detailsOwner} setDetailOwner={setDetailsOwner}/> : module === 'c2' ? <AIAnalysisPanel job={j}/> : <p role="alert">Unsupported analysis module.</p>}
