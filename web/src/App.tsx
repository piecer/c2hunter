...
function SensorPCAPs() {
  ...
  return <><header className="header-actions"><div><p className="eyebrow">CAPTURE ARCHIVE</p><h1>Sensor PCAPs</h1><p className="muted">Download finalized packet captures uploaded by enrolled sensors.</p></div></header>
    <AsyncState query={query}>{data => items(data).length ? <table aria-label="Sensor PCAP archives"><thead><tr><th>Analysis</th><th>Sensor</th><th>File</th><th>Size</th><th>Uploaded</th><th>Action</th></tr></thead><tbody>{items(data).map(segment => <tr key={segment.id}><td>{segment.analysis_job_id ? <Link to={`/analyses/${segment.analysis_job_id}`}>{segment.analysis_job_id}</Link> : 'Legacy capture'}</td><td>{segment.sensor_name ?? segment.sensor_id}</td><td><code>{segment.filename}</code></td><td>{formatBytes(segment.size_bytes)}</td><td>{fmt(segment.uploaded_at)}</td><td><button disabled={download.isPending} onClick={() => download.mutate(segment)}>{download.isPending ? 'Downloading…' : `Download ${segment.filename}`}</button></td></tr>)}</tbody></table> : <div className="state">No sensor PCAP archives</div>}</AsyncState>
    {download.error && <p role="alert" className="error-text">{download.error.message}</p>}
  </>;
}
