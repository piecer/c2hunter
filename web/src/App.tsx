  ...
  return <section className="panel compact candidate-enrichment" aria-labelledby="candidate-validation-title">
    ...
    <div className="grid compact">
      ...
      <div className="subpanel">
        <div className="section-heading"><div><h3>외부 위협 인텔리전스</h3>{threatIntel && <span className={`workflow-badge ti-${(threatIntel.status ?? 'COMPLETED').toLowerCase()}`}>{threatIntel.status === 'PENDING' ? '자동 조회 중' : threatIntel.origin === 'AUTO' ? '자동 조회 완료' : '수동 조회 완료'}</span>}</div><button className="secondary" disabled={threatIntelMutation.isPending} onClick={() => threatIntelMutation.mutate()}>{threatIntelMutation.isPending ? '조회 중…' : '외부 TI 다시 조회'}</button></div>
