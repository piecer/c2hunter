...
type ThreatIntelProvider = { status: 'OK' | 'ERROR' | 'AUTH_ERROR' | 'RATE_LIMITED' | 'NOT_CONFIGURED'; malicious?: number; suspicious?: number; harmless?: number; reputation?: number; abuse_confidence_score?: number; country_code?: string | null; total_reports?: number; error?: string; retry_after?: string };
  ...
  return <section className="panel compact candidate-enrichment" aria-labelledby="candidate-validation-title">
    ...
    <div className="grid compact">
      ...
      <div className="subpanel">
        <div className="section-heading"><h3>외부 위협 인텔리전스</h3><button className="secondary" disabled={threatIntelMutation.isPending} onClick={() => threatIntelMutation.mutate()}>{threatIntelMutation.isPending ? '조회 중…' : '외부 TI 조회'}</button></div>
        {!threatIntel && <p className="muted">Run an on-demand lookup against the configured VirusTotal and AbuseIPDB providers.</p>}
