...
type DashboardData = {
  ...
  candidates: { total: number; critical: number; high: number; medium: number; low: number; new_24h: number };
...
type CandidateFilters = { severity: string; verdict: string; minimumScore: string; includeSuppressed: boolean; sort: string };
const defaultCandidateFilters: CandidateFilters = { severity: '', verdict: '', minimumScore: '0', includeSuppressed: false, sort: '-score' };
...
function CandidateEnrichmentPanel({ candidate, onUpdated }: { candidate: Candidate; onUpdated: () => Promise<unknown> }) {
  ...
  const errors = [verdictMutation.error, threatIntelMutation.error, mispMutation.error].filter(Boolean) as Error[];
