...
export type PatternReport = {
  version: string; summary: Record<string, unknown>; issues?: PatternIssue[];
...
function Notes({ values, issue }: { values: unknown; issue?: PatternIssue }) {
  const language = useReportLanguage();
  const items = Array.isArray(values) ? values : [];
  return <ul>{items.slice(0, 4).map((value, index) => <li key={index}>{issue ? evidenceText(language, value, issue) : translateProse(language, value)}</li>)}{items.length > 4 && <li>{items.length - 4} {choose(language, 'additional notes omitted from this view.', '개의 추가 참고 사항이 이 화면에서 생략되었습니다.')}</li>}</ul>;
}
export default function NetworkPatternReport({ report }: { report: PatternReport }) {
  ...
  return <section className="panel network-report" aria-labelledby="overall-network-heading">
    ...
    <section aria-label={t('Analysis coverage', '분석 범위')}><h3>{t('Coverage and omissions', '분석 범위 및 생략')}</h3>
      ...
      <ul>{warnings.slice(0, 8).map((warning, index) => <li key={index}>{warningLabel(language, warning)}</li>)}{warnings.length > 8 && <li>{warnings.length - 8} {t('additional coverage warnings omitted from this view.', '개의 추가 분석 범위 경고가 이 화면에서 생략되었습니다.')}</li>}</ul><Notes values={report.limitations}/>
    </section>
