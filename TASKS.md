...
| REQ-PCP-002 | §14.2 | Upload/LIVE/reanalysis source, 전체/후보/applied-filter PCAP, nested filter, source integrity, bounded hybrid sync/durable-async PCAP/PCAPNG export lifecycle |
...
| [x] | P4-004 | source provenance 기반 bounded hybrid filtered PCAP/PCAPNG export와 Stage 9 offline structural index 구현 | REQ-PCP-002 | P2-009,P3-008 | shared executor, exact ≤32 MiB/≤100k trusted sync admission, unknown/large durable async, principal idempotency-before-capacity, lease/retry/recovery/cancel, metadata-last publication, three retention bounds/orphan cleanup, sync compatibility; Stage 9 canonical upload index는 source-bound/atomic/public schema 없음 |
| [x] | P4-004A | Stage 10 finalized LIVE segment optional background structural metadata | REQ-PCP-002 | P4-004 | request marker/admission best-effort, dedicated bounded lease worker/reconcile/cleanup/metrics, canonical source deletion cascade, public schema 없음, Stage 12까지 sync/async export lookup 0회; Stage 11 posting/Stage 12 range-read 제외, live backend gate는 opt-in |
| [x] | P4-004B | Stage 11 source-bound candidate/filter packet-posting generation과 내부 safe-superset facade | REQ-PCP-002, REQ-PER-001 | P4-004A | bounded posting build/backfill/lease worker, immutable source+Stage 10 parent+contract+digest binding, Analysis/Candidate ordinal 후보 view와 보수적 `None` fallback, public schema 없음, sync/async export posting lookup 0회; Stage 12 range read/coalescing 제외, PostgreSQL+MinIO live gate는 opt-in이며 미실행 |
| [x] | P4-004C | Stage 12 source-bound indexed sparse PCAP export와 안전한 coalesced range read rollout | REQ-PCP-002, REQ-PER-001 | P4-004B | active sparse path는 full source read 없이 exact version-pinned ranges만 읽고 dense/오류는 sequential fallback 1회; shadow는 sequential authoritative, bounded metrics/benchmark/rollback off, public schema 변화 없음; PostgreSQL+MinIO live gate는 opt-in이며 미실행 |
...
- **PCAP 개인정보/용량:** opt-in, RBAC/authenticated download/audit, bounded hybrid admission과 durable PostgreSQL export queue를 적용. Terminal age/count/artifact-byte 및 unreferenced staging age를 독립 제한하고 `sync_only`에서도 accepted async work를 drain한다.
