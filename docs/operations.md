...
Stage 9 builds a derived, best-effort structural index only while accepting a canonical offline `PCAP_UPLOAD`; upload success and replay safety do not depend on index availability. After upload/read-back verification, PostgreSQL stores the authoritative source/job ID, exact `captures/{job_id}.pcap` key, immutable backend version ID, byte size, and SHA-256 in `pcap_capture_source_versions`. The builder binds the generation to that durable row plus capture format, schema version, and parser contract version after full-source size/SHA/format and packet-structure verification. Publication and lookup revalidate the durable row inside the database transaction; they do not call MinIO for a mutable-object precheck while holding database locks. Source retention/deletion removes the version row and index generations transactionally while preserving the canonical object-cleanup outbox. The metadata owner switch remains atomic so partial generations are never readable. Canonical source deletion owns deletion of READY and staging generations. The index has no public API/OpenAPI schema and sync/async exports do not query it until Stage 12. There is no LIVE/reanalysis, posting-index, or range-read behavior from Stages 10–12.

Bound resource use with `C2HUNTER_PCAP_OFFSET_INDEX_MAX_PACKETS` (defaults to the upload packet ceiling), `C2HUNTER_PCAP_OFFSET_INDEX_MAX_INTERFACES` (default 4096), and `C2HUNTER_PCAP_OFFSET_INDEX_BATCH_SIZE` (default 1000, maximum 10000). The repository cleanup seam for abandoned staging generations is bounded by `C2HUNTER_PCAP_OFFSET_INDEX_STAGING_MAX_AGE_SECONDS` (default 3600) and `C2HUNTER_PCAP_OFFSET_INDEX_CLEANUP_BATCH_SIZE` (default 100, maximum 10000); these settings do not imply that Stage 10 background indexing exists. The real PostgreSQL+MinIO publication/read/delete test is opt-in via `C2HUNTER_RUN_STORAGE_INTEGRATION=1` plus the database and S3 connection variables; a default skip is not live verification.
