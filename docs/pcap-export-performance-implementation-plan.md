...
### Stage 12 implemented boundary and measurement

Stage 12 is internal only: READY posting ordinals are mapped through their exact READY structural parent into immutable-version-pinned packet locators and a bounded request-wide range plan. Active sparse success reads only selected/coalesced byte ranges and never opens or fetches the full retained source. Dense plans, absent/stale/corrupt metadata, exact limit+1, short read, provider outage, version drift, or final source/parent/posting fence failure atomically discard the provisional batch and run the whole manifest sequentially exactly once. Shadow always publishes the sequential artifact; indexed output is comparison-only. There is no public request, response, OpenAPI, UI, or persistence schema change.

The trust boundary intentionally cannot recompute a full-source SHA-256 from sparse bytes. It instead requires the authoritative immutable backend version plus exact object key, byte size and stored SHA-256, exact structural/posting identities, provider-enforced range identity, and a final metadata fence after decoding. Any doubt falls back. The one final deterministic Stage 12 benchmark run (`--stage12 --packets 128 --seed 20260720`) used the production posting builder, internal factory, and `MemoryRepository.read_capture_range`. The sparse case selected 3 packet payloads/301 bytes from 15,576 source bytes, admitted and attempted 3 ranges, received/fetched 301 bytes (1.9324601951720595% of source; 15,275 final-execution bytes avoided), and measured 0.001594114932231605 s sequential versus 0.011503273039124906 s indexed with a 60,870,656-byte process peak RSS. **On this single host and run the indexed sparse path was slower; this is explicitly not a speedup claim.** The clustered case reduced 8 candidate payload requests to 1 admitted/attempted coalesced range and fetched 860/15,576 bytes. Multi-source LIVE reported 2 sources and range reads from both IDs; PCAPNG reported 2 interfaces. Sparse, clustered, multi-source, and PCAPNG produced complete indexed artifacts with exact SHA-256/size/packet-count parity. Dense and exact range-limit+1 candidates were rejected before complete plan admission with `resource_limit`: no range calls were attempted, final fallback read the 15,576-byte source, indexed artifact/parity are unavailable (`null`), and final bytes saved are 0. Injected short read admitted a 3-range/301-byte plan, attempted 1 range/90 bytes, received 89 provisional bytes, then read the full source on fallback (15,665 total fetched); injected version drift attempted 1 range/90 bytes, received 0, then read the full 15,576-byte source. Both have `indexed=null`, `parity=null`, and `source_bytes_saved=0`. In schema version 3, candidate request/selected-byte counts describe runtime plan input; nullable coalesced/planned fields describe complete plan admission; attempted/received fields come from the repository spy; `fetched_bytes` and `source_bytes_saved` describe the authoritative final execution; and `final_artifact` identifies fallback output without copying it into `indexed`. The generated report was parsed for these values and then removed; the opt-in PostgreSQL+MinIO gate was not run.
